...
from transformer_base import run_clm_rl, run_summarization_rl
from rp_utils import DynamicModel, RelationData, RelationSentence

# SUPPORTED_TASKS['relation-generator'] = {
...
class RelationGenerator(RelationModel):
    ...
    def fit(self, path_train: str, path_dev: Optional[str] = None):
        ...
        model_args = run_clm_rl.ModelArguments(model_name_or_path=self.model_name)
        run_clm_rl.main(
...
class NewRelationGenerator(RelationModel):
    ...
    def generate(
        ...
        while len(sents) < num:
            ...
            for o in outputs:
                raw.append(run_summarization_rl.encode_to_line(x=prompt, y=o))
                try:
