...
class GenerateRHTEncoder(GenerateEncoder):
    def encode_x(self, r: str) -> str:
        return f"Relation : {r} ."
    ...
    def encode_triplet(self, sent: RelationSentence) -> str:
        s, r, o = sent.as_tuple()
        return f"Head Entity : {s} , Tail Entity : {o} . Context : {sent.text}"

    def decode_triplet(self, text: str, label: str) -> RelationSentence:
        front, back = text.split(" , Tail Entity : ")
        _, head = front.split("Head Entity : ")
        tail, context = back.split(" . Context : ")
        return RelationSentence.from_spans(context, head, tail, label)
    ...
    def decode_y(self, text: str, label: str) -> RelationSentence:
        ...
        front, back = text.split(" Head Entity : ")
        label = self.decode_x(front)
        return self.decode_triplet("Head Entity : " + back, label)
...
class GeneratePromptEncoder(GenerateEncoder):
    def encode_x(self, r: str) -> str:
        return f"Produce the head and tail entities based on the relation type. Subsequently, formulate a sentence encompassing the head entity, relation, and tail entity. Relation : {r} ."
...
class GeneratorMultiPromptEncoder(GeneratePromptEncoder):
    def encode_x(self, r: str) -> str:
        candidates = [
        'Produce the head and tail entities based on the relation type. Subsequently, formulate a sentence encompassing the head entity, relation, and tail entity.', 
        'Generate the head and tail entities using the relationship category. Afterward, create a sentence that includes the head entity, the relationship, and the tail entity.', 
        'Identify the entities that are related by the given type of relation. Then, write a sentence that includes the related entities and the type of relation.',
        'Find the entities that have the specified relation between them. Next, construct a sentence that contains the entities and the relation.', 
        'Generate the head and tail entities using the type of relationship. Then, construct a sentence that covers the head entity, relation, and the tail entity.']
        return random.sample(candidates, 1)[0] + f" Relation : {r} ."

class ExtractEncoder(Encoder):
    def encode_x(self, text: str) -> str:
        return f"Context : {text}"
    def encode_entities(self, sent: RelationSentence): 
        s, r, o = sent.as_tuple()
        return f"Head Entity : {s} , Tail Entity : {o} , Relation : "
...
def select_encoder(name: str) -> Encoder:
    mapping: Dict[str, Encoder] = dict(
        ...
        generate_rht=GenerateRHTEncoder(), 
        generate_prompt=GeneratePromptEncoder(),
        generate_multi=GeneratorMultiPromptEncoder(),
    )
...
def test_entity_prompts(
    path: str = "outputs/data/splits/zero_rte/wiki/unseen_10_seed_0/test.jsonl", limit: int = 100
):
