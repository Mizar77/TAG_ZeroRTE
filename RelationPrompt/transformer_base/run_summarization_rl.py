...
from transformers import (AutoConfig, AutoModelForSeq2SeqLM, AutoTokenizer,
                          ...
                          Seq2SeqTrainer, Seq2SeqTrainingArguments, set_seed, BartForConditionalGeneration, BartModel, BartConfig)
...
class Model(BartForConditionalGeneration):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
            reward = reward.unsqueeze(-1).expand_as(labels).contiguous().view(-1)
            reward = torch.where(labels.view(-1) == -100, torch.tensor(0.).cuda(), reward)
            masked_lm_loss = (masked_lm_loss * reward).sum() / reward.sum()
...
def main(
    ...
    tokenizer_kwargs = data_args.tokenizer_kwargs or {}
    print(dict(tokenizer_kwargs=tokenizer_kwargs))
    ...
    print(dict(tokenizer=tokenizer))
